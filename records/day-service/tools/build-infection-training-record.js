...
const ex = [
  ...
  ['研修概要', '講師・進行担当者（所属・職種・氏名）', '感染対策担当者・看護職員・職員A'],
  ...
  ['確認', '管理者確認（氏名・確認日）', ['氏名：管理者・職員B', '確認日：20XX年6月13日']],
  ['添付', '添付資料', '■資料　■参加者名簿　■テスト・アンケート　□その他'],
].map(x => [...x, 480]);

const exRoster = [
  ['管理者', '職員B', '集合', '20XX/6/10', '署名済', ''],
  ['看護職員', '職員A', '集合', '20XX/6/10', '署名済', '講師'],
...
const children = [
  ...
  brk(),
  // 2 記入例
  h1('【記入例】感染症の予防及びまん延の防止のための研修 記録（通所介護・デイサービス）'),
  p('※記入例の事業所名・職員名・番号・日付はすべて架空です（職員名は「職員A」などの記号で示しています）。記入の粒度の参考としてご覧ください。■はチェックを入れた項目です。', { size: 18, after: 120 }),
  recordTable(ex),
  brk(),
  h2('【記入例】別表　参加者名簿（抜粋）'),
...
const doc = new Document({
  ...
  title: '通所介護 デイサービス 感染症の予防及びまん延の防止のための研修 記録 様式 ひな形 記入例付き',
