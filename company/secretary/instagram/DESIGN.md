...
- ただし薄くしすぎると読めなくなる。ページ番号は当初 `#94897E`（比2.89）→ `#6F6357`（比4.94）と直したが、それでも実機で視認しづらかったため、最終的に**本文と同じ `#4A4038`（比8.54）／44px／太字**にした
- **教訓：補助的な文字ほど大きく濃くする。** 画面や明るさによって、数値上は足りていても読めないことがある
